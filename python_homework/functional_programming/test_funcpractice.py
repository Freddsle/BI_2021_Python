...
def test_sequential_map():
    ...
    assert functional.sequential_map(lambda x: x**2, []) is None
...
def test_consensus_filter():
    ...
    assert functional.consensus_filter(lambda x: x > 0, []) is None
...
def test_conditional_reduce():
    ...
    assert functional.conditional_reduce(lambda x: x < 5, lambda x, y: x + y, []) is None
